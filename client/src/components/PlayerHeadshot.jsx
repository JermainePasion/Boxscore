...
export default function PlayerHeadshot({ playerId, teamId, season, className }) {
  ...
  // A cached image can finish loading before React wires up onLoad, so the
  // event never fires and the spinner hangs. After mount/src change, check
  // whether the image is already complete and sync state to it.
  useEffect(() => {
  ...
  }, [idx, playerId, teamId, season]) // re-check whenever the src changes
